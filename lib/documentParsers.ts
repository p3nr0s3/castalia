...
/**
 * Encapsulates a PDF document for embedded view and AI analysis.
 */
export function parsePdfDocument(file: File): ParsedDocument {
  const url =
    typeof URL !== "undefined" && typeof URL.createObjectURL === "function"
...
/**
 * Universal dispatcher that automatically detects format from file extension.
 */
export async function parseDocumentFile(file: File): Promise<ParsedDocument> {
  const name = file.name.toLowerCase();
  if (name.endsWith(".epub")) {
    const doc = await parseEpub(file, file.name.replace(/\.epub$/i, ""));
    doc.filesize = file.size;
    return doc;
  }
  if (name.endsWith(".cbz") || name.endsWith(".cbr")) {
    const doc = await parseComic(file, file.name);
    doc.filesize = file.size;
    return doc;
  }
  ...
  if (name.endsWith(".txt") || name.endsWith(".md") || name.endsWith(".markdown")) {
    ...
    const doc = await parseTextDocument(text, file.name);
    doc.filesize = file.size;
    return doc;
  }
  throw new Error(`Format file "${file.name}" belum didukung. Harap gunakan EPUB, CBZ, CBR, PDF, TXT, atau MD.`);
}
